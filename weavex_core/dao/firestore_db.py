...
class WeavexDao(ABC):
    ...
    def get_checkpoint(
        ...
        *string*, plus the reserved entries "step_context", "context",
        ...
        field NAMES, not "."-delimited Firestore field paths. Names are escaped
        for you, so a stepId containing a dot is read as that literal key rather
        than as a nested lookup. Pass None (or an empty iterable) to read the
        whole document.
