...
def run_test():
    ...
    try:
        ...
        csv_uri = store.upload_report("proj_test", "job_001", "run_001", csv_content, "csv")
    ...
    try:
        txt_uri = store.upload_report("proj_test", "job_001", "run_001", b"line1\nline2", "txt")
    ...
    try:
        xlsx_uri = store.upload_report("proj_test", "job_001", "run_001", b"fake_xlsx_bytes", "xlsx")
    ...
    try:
        downloaded = store.download_report("proj_test", "job_001", csv_uri)
    ...
    try:
        url = store.get_report_presigned_url("proj_test", "job_001", csv_uri)
    ...
    try:
        url_custom = store.get_report_presigned_url("proj_test", "job_001", csv_uri, expiration_seconds=7200)
    ...
    try:
        store.download_report("wrong_proj", "job_001", csv_uri)
    ...
    try:
        store.get_report_presigned_url("proj_test", "wrong_job", csv_uri)
        print("    FAIL: Expected PermissionError but none was raised")
    except PermissionError:
        print("    Security check passed ✓")
    except Exception as e:
    ...
    try:
        store.upload_report("proj_test", "job_001", "run_001", b"data", "pdf")
    ...
    try:
        store.download_report("proj_test", "job_001", "not-a-gs-uri")
        print("    FAIL: Expected ValueError but none was raised")
    except ValueError:
